...
st.markdown("""
    ...
    [data-testid="stAppViewContainer"] {
        background-color: #ffc2d1;  /* white */
    }

    /* SIDEBAR */
    [data-testid="stSidebar"] {
        background-color: #ff8fab;  /* pink */
        color: black;
    }
    ...
    /* CUSTOM WARNA EXPANDER */
    div[data-testid="stExpander"] > details > summary {
        background-color: #e14389;  /* pink */
        color: white;
        padding: 12px;
        font-weight: bold;
        border-radius: 8px;
    ...
    summary::marker {
        color: pink;  /* Warna ikon segitiga */
    }
    </style>
...
# Navigasi sidebar
with st.sidebar:
    selected = option_menu("NIO FARM", 
                           ['Profile', 'Data Transaksi', 'Data Persediaan', 'Data Beban', 'Data Modal', 'Neraca Saldo Periode Sebelumnya', 'Jurnal Umum', 'Buku Besar', 'Neraca Saldo', 'Neraca Lajur', 'Laporan Laba Rugi', 'Laporan Perubahan Modal', 'Laporan Posisi Keuangan', 'Jurnal Penutup', 'Neraca Saldo Setelah Penutup'],  # Pilihan menu
                            default_index=0)
...
if selected == 'Profile':
    ...
    if st.session_state.df_data_transaksi.empty:
        ...
        st.session_state.df_data_transaksi["Nominal (Rp)"] = pd.to_numeric(st.session_state.df_data_transaksi["Nominal (Rp)"], errors="coerce").fillna(0)
...
# Halaman Buku Besar
    ...
    # Dropdown akun
    akun = st.selectbox("Pilih Akun Buku Besar", 
        ["Kas", "Persediaan", "Perlengkapan", "Peralatan", "Penjualan", "Beban Gaji"]
    )

    # Mapping nama akun ke variabel session_state
    akun_dict = {
        "Kas": "df_kas",
        "Persediaan": "df_persediaan",
        "Perlengkapan": "df_perlengkapan",
        "Peralatan": "df_peralatan",
        "Penjualan": "df_penjualan",
        "Beban Gaji": "df_beban_gaji"
    }
    ...
    if akun_dict[akun] not in st.session_state:
        st.session_state[akun_dict[akun]] = pd.DataFrame(columns=columns)

    st.write(f"Menampilkan Buku Besar {akun}")

    # Form tambah transaksi
    with st.form(f"form_tambah_transaksi_{akun.lower().replace(' ', '_')}", clear_on_submit=True):
        st.write("Tambah Transaksi")
        tanggal = st.date_input("Tanggal Transaksi", format="DD/MM/YYYY")
        nama_akun = st.text_input("Nama Akun")
        debit = st.number_input("Debit (Rp)", min_value=0, step=1000)
        ...
        if tambah:
            if nama_akun.strip() == "" or (debit == 0 and kredit == 0):
                ...
                st.session_state[akun_dict[akun]] = pd.concat(
                    [st.session_state[akun_dict[akun]], pd.DataFrame([row])],
                    ignore_index=True
                )
                st.success("Transaksi berhasil ditambahkan.")
    ...
    # Tampilkan tabel
    df = st.session_state[akun_dict[akun]]
    ...
    if df.empty:
        ...
        gb.configure_column("Tanggal", editable=False)
        grid_options = gb.build()

        st.markdown(f"Tabel {akun}")
        grid_response = AgGrid(
            df,
            gridOptions=grid_options,
            update_mode=GridUpdateMode.VALUE_CHANGED,
        ...
        updated_data = grid_response["data"]
        st.session_state[akun_dict[akun]] = pd.DataFrame(updated_data)

        # Hitung total
        st.session_state[akun_dict[akun]]["Debit (Rp)"] = pd.to_numeric(
            st.session_state[akun_dict[akun]]["Debit (Rp)"], errors="coerce"
        ).fillna(0)

        st.session_state[akun_dict[akun]]["Kredit (Rp)"] = pd.to_numeric(
            st.session_state[akun_dict[akun]]["Kredit (Rp)"], errors="coerce"
        ).fillna(0)

        total_row = {
            "Tanggal": "",
            "Nama Akun": "Total",
            "Debit (Rp)": st.session_state[akun_dict[akun]]["Debit (Rp)"].sum(),
            "Kredit (Rp)": st.session_state[akun_dict[akun]]["Kredit (Rp)"].sum()
        }

        df_tampil = pd.concat(
            [st.session_state[akun_dict[akun]], pd.DataFrame([total_row])],
            ignore_index=True
        )
    ...
    if st.session_state.df_neraca_saldo.empty:
        ...
        # Halaman Neraca Lajur
elif selected == 'Neraca Lajur':
    st.subheader('Neraca Lajur')
    st.markdown('Periode 30 April 2025')
        
    columns = ["No", "Nama Akun", "Debit (Rp)", "Kredit (Rp)"]

    if "df_neraca_lajur" not in st.session_state:
        st.session_state.df_neraca_lajur = pd.DataFrame(columns=columns)

    # Formulir untuk menambah transaksi
    with st.form("form_tambah_transaksi_neraca_lajur", clear_on_submit=True):
        st.write("Tambah Transaksi")
        nama_akun = st.text_input("Nama Akun")
        debit = st.number_input("Debit (Rp)", min_value=0, step=1000)
        kredit = st.number_input("Kredit (Rp)", min_value=0, step=1000)
    ...
    with col2:
        ...
        if tambah:
            ...
            if nama_akun.strip() == "" or (debit == 0 and kredit == 0):
                ...
                # Tambah transaksi ke DataFrame di session_state
                nomor = len(st.session_state.df_neraca_lajur) + 1
                row = {
                    "No": nomor,
                    "Nama Akun": nama_akun,
                    "Debit (Rp)": debit if debit > 0 else None,
                ...
                # Update DataFrame di session_state
                st.session_state.df_neraca_lajur = pd.concat(
                    [st.session_state.df_neraca_lajur, pd.DataFrame([row])],
                    ignore_index=True
                )
                st.success("Transaksi berhasil ditambahkan.")
    ...
    if st.session_state.df_neraca_lajur.empty:
        st.info("Tabel belum memiliki transaksi. Tambahkan transaksi di atas.")
    else:
        gb = GridOptionsBuilder.from_dataframe(st.session_state.df_neraca_lajur)
        gb.configure_default_column(editable=True)  # Semua kolom bisa diedit
        gb.configure_column("No", editable=False)  # Kolom "No" tidak bisa diedit
        grid_options = gb.build()

        st.markdown("Tabel Neraca Lajur")
        grid_response = AgGrid(
            st.session_state.df_neraca_lajur,
            gridOptions=grid_options,
            update_mode=GridUpdateMode.VALUE_CHANGED,
            fit_columns_on_grid_load=True    
        )
        ...
        updated_data = grid_response["data"]
        st.session_state.df_neraca_lajur = pd.DataFrame(updated_data)

        st.session_state.df_neraca_lajur["Debit (Rp)"] = pd.to_numeric(st.session_state.df_neraca_lajur["Debit (Rp)"], errors="coerce").fillna(0)
        st.session_state.df_neraca_lajur["Kredit (Rp)"] = pd.to_numeric(st.session_state.df_neraca_lajur["Kredit (Rp)"], errors="coerce").fillna(0)

        # Tambahkan total debit dan kredit
        total_row = {
            "No": "",
            "Nama Akun": "Total",
            "Debit (Rp)": st.session_state.df_neraca_lajur["Debit (Rp)"].sum(skipna=True),
            "Kredit (Rp)": st.session_state.df_neraca_lajur["Kredit (Rp)"].sum(skipna=True)
        }
    ...
    if st.session_state.df_laporan_laba_rugi.empty:
        ...
        total_row = {
            "No": "",
            "Nama Akun": "Total",
            "Debit (Rp)": st.session_state.df_laporan_laba_rugi["Debit (Rp)"].sum(skipna=True),
            "Kredit (Rp)": st.session_state.df_laporan_laba_rugi["Kredit (Rp)"].sum(skipna=True)
        }
    ...
    if st.session_state.df_laporan_perubahan_modal.empty:
        ...
        total_row = {
            "No": "",
            "Nama Akun": "Total",
            "Debit (Rp)": st.session_state.df_laporan_perubahan_modal["Debit (Rp)"].sum(skipna=True),
            "Kredit (Rp)": st.session_state.df_laporan_perubahan_modal["Kredit (Rp)"].sum(skipna=True)
        }
    ...
    if st.session_state.df_laporan_posisi_keuangan.empty:
        ...
        st.session_state.df_laporan_posisi_keuangan["Debit (Rp)"] = pd.to_numeric(st.session_state.df_laporan_posisi_keuangan["Debit (Rp)"], errors="coerce").fillna(0)
        st.session_state.df_laporan_posisi_keuangan["Kredit (Rp)"] = pd.to_numeric(st.session_state.df_laporan_posisi_keuangan["Kredit (Rp)"], errors="coerce").fillna(0)

        # Tambahkan total debit dan kredit
        total_row = {
            "No": "",
            "Nama Akun": "Total",
            "Debit (Rp)": st.session_state.df_laporan_posisi_keuangan["Debit (Rp)"].sum(skipna=True),
            "Kredit (Rp)": st.session_state.df_laporan_posisi_keuangan["Kredit (Rp)"].sum(skipna=True)
        }
